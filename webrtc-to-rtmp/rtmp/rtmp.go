...
const pipelinestring = "appsrc is-live=true do-timestamp=true name=videosrc ! queue ! h264parse ! nvh264dec ! cudaconvert ! cudascale ! video/x-raw(memory:CUDAMemory), width=2560, height=1440 ! nvh264enc zerolatency=true ! h264parse ! video/x-h264, stream-format=(string)avc ! muxer.   appsrc is-live=true do-timestamp=true name=audiosrc ! queue ! opusparse ! opusdec ! audioconvert ! audioresample ! faac ! muxer.  flvmux name=muxer ! rtmp2sink sync=false location=%s"

//const pipelinestring = "appsrc is-live=true do-timestamp=true name=videosrc ! h264parse ! video/x-h264,stream-format=(string)avc, width=1280, height=720, framerate=15/1 ! muxer.   appsrc is-live=true do-timestamp=true name=audiosrc ! opusparse ! opusdec ! audioconvert ! audioresample ! faac ! muxer.  flvmux name=muxer ! rtmp2sink sync=false location=%s"
